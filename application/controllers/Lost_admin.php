		...
		if($this->input->post('submit',TRUE)=='Submit')
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($get ->num_rows()>0)
				{
					...
					$config['smtp_user'] = 'fanitheresahtb@gmail.com';//isi dengan gmail
					$config['smtp_pass'] = 'meime1010'; //isi dengan password
					$config['crlf'] = "\r\n";
					$config['newline'] = "\r\n";
					...
					$key = md5(md5(time()));
					$this->email->from('',"Olshopku");
					$this->email->to($this->input->post('email', TRUE));
					$this->email->subject('Reset Password');
					$this->email->message(
						'Apakah Anda lupa dengan password anda? silahkakn klik <a href="'.base_url().'lost_admin/reset/'.$key.'">disini</a>. Jika anda tidak merasa request fitur ini, silahkan abaikan pesan ini'
					);
